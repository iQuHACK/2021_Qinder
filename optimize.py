...
def general_optimize(compat_matrix, pairing_cost, double_pairing_cost, numruns):
	Q = defaultdict(float)
...
def general_optimize_iterated(compat_matrix, mps_matrix):
	...
	pairing_cost = -0.1 # "cost" assigned for each pairing (should be negative)
	double_pairing_cost = 0.6 # "cost" assigned for each person who is paired with two people (should be positive)
	mps_pairing_cost = 0.6 # "cost" assigned for each person who is paired with someone who is not an mps

	compat_matrix = compat_matrix * mps_matrix - mps_matrix * mps_pairing_cost

	# Run sampler
	N = compat_matrix.shape[0] / 2
	result = general_optimize(compat_matrix, pairing_cost, double_pairing_cost, numruns)
	print(result)
